...
To reference Figure \@ref(fig:cars-plot), use `\@ref(fig:cars-plot)` if `cars-plot` is the name of the chunk.

```{r cars-plot, fig.height=4, fig.width=5, fig.cap="The cars data."}
plot(cars, pch=16)
```

You can also reference equations, for instance the mean in Equation \@ref(eq:mean), using `\@ref(eq:mean)` in the text and `(\#eq:mean)` in the equation block.
...
Reference tables like Table \@ref(tab:mtcars) using `\@ref(tab:mtcars)` if `mtcars` is the name of the chunk.
